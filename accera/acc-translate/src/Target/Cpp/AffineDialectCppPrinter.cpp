...
void AffineDialectCppPrinter::printAffineMemCpy() {
  // general memcpy
  os << "template <typename T>\n";
  os << "__forceinline__ " << printer->deviceAttrIfCuda() << "\n";
  os << "void " << AffineDialectCppPrinter::affineMemCpyStr;
  os << "(T *dst, const T *src, size_t cnt) {\n";
  os << "  #pragma unroll\n";
  os << "  for (size_t i = 0; i < cnt; i++) {\n";
  os << "    dst[i] = src[i];\n";
  os << "  }\n";
  os << "}\n";

  const char *fp16_t = printer->float16T();
  const char *fp16_vec_t = printer->float16VecT();

  // float16 vector load
  os << "__forceinline__ " << printer->deviceAttrIfCuda() << "\n";
  os << "void " << AffineDialectCppPrinter::affineVecLoadFp16Str;
  os << "(" << fp16_vec_t << " *dst_vec, "
     << "const " << fp16_t << " *src, "
     << "size_t cnt) {\n";
  os << "  const " << fp16_vec_t << " *src_vec = reinterpret_cast<const "
     << fp16_vec_t << " *>(src);\n";
  os << printer->pragmaUnroll() << "\n";
  os << "  for (size_t i = 0; i < cnt / 2; i++) {\n";
  os << "    dst_vec[i] = src_vec[i];\n";
  os << "  }\n";
  os << "  if (cnt % 2) {\n";
  os << "    " << fp16_t << " *dst = reinterpret_cast<" << fp16_t
     << " *>(dst_vec);\n";
  os << "    dst[cnt - 1] = src[cnt - 1];\n";
  os << "  }\n";
  os << "}\n";

  // float16 vector store
  os << "__forceinline__ " << printer->deviceAttrIfCuda() << "\n";
  os << "void " << AffineDialectCppPrinter::affineVecStoreFp16Str;
  os << "(" << fp16_t << " *dst, "
     << "const " << fp16_vec_t << " *src_vec, "
     << "size_t cnt) {\n";
  os << "  " << fp16_vec_t << " *dst_vec = reinterpret_cast<" << fp16_vec_t
     << " *>(dst);\n";
  os << printer->pragmaUnroll() << "\n";
  os << "  for (size_t i = 0; i < cnt / 2; i++) {\n";
  os << "    dst_vec[i] = src_vec[i];\n";
  os << "  }\n";
  os << "  if (cnt % 2) {\n";
  os << "    const " << fp16_t << " *src = reinterpret_cast<const " << fp16_t
     << " *>(src_vec);\n";
  os << "    dst[cnt - 1] = src[cnt - 1];\n";
  os << "  }\n";
  os << "}\n";
...
void AffineDialectCppPrinter::printAffineCeilDiv() {
  os << "__forceinline__ " << printer->deviceAttrIfCuda() << "\n";
  os << affineIdxTypeStr << " " << AffineDialectCppPrinter::affineCeilDivStr
     << "(" << affineIdxTypeStr << " n, " << affineIdxTypeStr << " d) {\n";

  os << "  return static_cast<int64_t>(std::ceil((float)n / (float)d));\n";
...
static std::string makeAffineIdxFuncName(StringRef funcBaseName, int idx) {
...
AffineDialectCppPrinter::printAffineMapFunc(AffineMap map,
  ...
  for (int resIdx = 0; resIdx < (int)(map.getNumResults()); resIdx++) {
    ...
    printAffineExpr(map.getResult(resIdx));
    os << ";\n";
    os << "return " << idxName << ";\n";
...
LogicalResult AffineDialectCppPrinter::printPrologue() {
  ...
  for (auto p : sortedMapFuncs) {
    printAffineMapFunc(p.first, p.second);
  }
  return success();
}
...
void AffineDialectCppPrinter::printAffineMapResultIndices(
    AffineMap map, Operation::operand_range origIndices,
  ...
  for (int idx = 0; idx < map.getNumResults(); idx++) {
...
LogicalResult AffineDialectCppPrinter::printMemRefAccessPtr(
    Value memRef, const SmallVector<StringRef, 4> &memIdxVars,
...
AffineDialectCppPrinter::printAffineApplyOp(AffineApplyOp affineApplyOp) {
  ...
  printAffineMapResultIndices(affineApplyOp.getAffineMap(),
                              affineApplyOp.getMapOperands(), memIdxVars);
...
AffineDialectCppPrinter::printAffineLoadOp(AffineLoadOp affineLoadOp) {
  ...
  printAffineMapResultIndices(affineLoadOp.getAffineMap(),
                              affineLoadOp.getMapOperands(), memIdxVars);

  std::string srcMemRefVal;
  RETURN_IF_FAILED(printMemRefAccessValue(affineLoadOp.getMemRef(), memIdxVars,
                                          srcMemRefVal));
...
AffineDialectCppPrinter::printAffineStoreOp(AffineStoreOp affineStoreOp) {
  ...
  printAffineMapResultIndices(affineStoreOp.getAffineMap(),
                              affineStoreOp.getMapOperands(), memIdxVars);

  std::string dstMemRefPtr;
  RETURN_IF_FAILED(printMemRefAccessPtr(affineStoreOp.getMemRef(), memIdxVars,
                                        dstMemRefPtr));
...
LogicalResult AffineDialectCppPrinter::printAffineVectorLoadOp(
  ...
  printAffineMapResultIndices(affineVecLoadOp.getAffineMap(),
                              affineVecLoadOp.getMapOperands(), memIdxVars);

  std::string srcMemRefPtr;
  RETURN_IF_FAILED(printMemRefAccessPtr(affineVecLoadOp.getMemRef(), memIdxVars,
                                        srcMemRefPtr));
  ...
  if (vecElemType.isa<Float16Type>()) {
    os << affineVecLoadFp16Str << "(" << vecVar << ", " << srcMemRefPtr << ", "
       << vecType.getNumElements() << ")";
  } else {
    return affineVecLoadOp.emitError("<<unsupported VectorType>>");
  }
...
LogicalResult AffineDialectCppPrinter::printAffineVectorStoreOp(
  ...
  printAffineMapResultIndices(affineVecStoreOp.getAffineMap(),
                              affineVecStoreOp.getMapOperands(), memIdxVars);

  std::string dstMemRefPtr;
  RETURN_IF_FAILED(printMemRefAccessPtr(affineVecStoreOp.getMemRef(),
                                        memIdxVars, dstMemRefPtr));
  ...
  auto vecElemType = vecType.getElementType();
  if (vecElemType.isa<Float16Type>()) {
    os << affineVecStoreFp16Str << "(" << dstMemRefPtr << ", " << vecVar << ", "
       << vecType.getNumElements() << ")";
  } else {
    return affineVecStoreOp.emitError("<<unsupported VectorType>>");
  }
...
void AffineDialectCppPrinter::checkDeadAffineOpsPass(Operation *op) {
  op->walk([&](Operation *subOp) {
    if (isa<AffineApplyOp, AffineLoadOp, AffineMaxOp, AffineMinOp,
            AffineVectorLoadOp>(subOp)) {
