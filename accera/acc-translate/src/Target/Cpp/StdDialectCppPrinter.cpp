...
static bool isSimpleCastOp(Operation *op) {
  return isa<FPExtOp>(op) || isa<FPTruncOp>(op) || isa<IndexCastOp>(op) ||
...
LogicalResult StdDialectCppPrinter::printCallOp(CallOp callOp) {
  ...
  printer->printDeclarationForOpResult(op);
...
LogicalResult StdDialectCppPrinter::printDimOp(memref::DimOp dimOp) {
  ...
  printer->printDeclarationForOpResult(dimOp.getOperation());
...
LogicalResult StdDialectCppPrinter::printLoadOp(memref::LoadOp loadOp) {
  ...
  RETURN_IF_FAILED(printer->printDeclarationForOpResult(loadOp.getOperation()));
  os << " = ";
  Value memref = loadOp.getMemRef();
  return printer->printMemRefAccess(memref, memRefType, indices);
}
...
LogicalResult StdDialectCppPrinter::printStoreOp(memref::StoreOp storeOp) {
  ...
  RETURN_IF_FAILED(printer->printMemRefAccess(memref, memRefType, indices));
  os << " = " << state.nameState.getName(storeOp.getValueToStore());
  return success();
}
