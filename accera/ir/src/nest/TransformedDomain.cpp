...
namespace loopnest
{
    namespace
    {
        ...
        std::optional<int64_t> IsPadExpression(const AffineExpression& expr)
        {
            // front-padding: d1 = d0 - size
            auto affineExpr = expr.GetAffineExpr();
            auto binOpExpr = affineExpr.dyn_cast_or_null<mlir::AffineBinaryOpExpr>();
            if (binOpExpr && binOpExpr.getKind() == mlir::AffineExprKind::Add)
            {
                auto rhs = binOpExpr.getRHS().dyn_cast_or_null<mlir::AffineConstantExpr>();
                if (rhs)
                {
                    int64_t size = rhs.getValue() * -1;
                    return size;
                }
            }
            return std::nullopt;
    ...
    TransformedDomain::TransformedDomain(const IterationDomain& domain)
    {
        ...
        for (auto r : ranges)
        {
            ...
            _indices[index] = { /*expr=*/{}, range, /*parents=*/{} };
        }
    }
    ...
    TransformedDomain::TransformedDomain(const TransformedDomain::AttributeKey& info)
    {
        ...
        for (const auto& [index, expr, range] : info.indices)
        {
            _indices[index] = { expr, range, /*parents=*/ {} };
            if (expr.IsIdentity())
            {
    ...
    Index TransformedDomain::Pad(const Index& index, int64_t size, mlir::MLIRContext* context)
    {
        ...
        // create the padded index. index(shape=[0, N)), paddedIndex(shape=[size, N+size))
        ...
        _indices[paddedIndex] = { {}, { size, parentRange.Size() + size }, { index } };
    ...
    SplitIndex TransformedDomain::Split(const Index& index, int64_t splitSize, mlir::MLIRContext* context)
    {
        ...
        if (parentRange.HasConstantEnd())
        {
            ...
            _indices[outer] = { {}, { parentRange.Begin(), parentEndValue, splitSize }, { index } };
        }
        else if (parentRange.HasIndexEnd())
        {
            auto parentEndIndex = parentRange.EndIndex();
            _indices[outer] = { {}, { parentRange.Begin(), parentEndIndex, splitSize }, { index } };
        }
        else
        {
            auto parentEndOperandIndex = parentRange.EndOperandIndex();
            _indices[outer] = { {}, { parentRange.Begin(), parentEndOperandIndex, splitSize }, { index } };
        }

        // Add inner index
        // Note: this will need to be clamped to parentRange's Begin and End during loop nest building
        _indices[inner] = { {}, { 0, splitSize, parentIncrement }, { index } };
    ...
    Index TransformedDomain::Skew(const Index& index, const Index& referenceIndex, mlir::MLIRContext* context)
    {
        ...
        _indices[skewedIndex] = { {}, { parentRange.Begin(), parentRange.Size() + skewFactor - 1 }, { index } };
    ...
    bool TransformedDomain::IsPaddedIndex(const Index& index) const
    {
        ...
        {
            if (IsPadExpression(GetIndexExpr(parent)))
            {
                return true;
            }
        }
    ...
    bool TransformedDomain::HasPaddedParentIndex(const Index& index) const
    {
        for (auto parent : GetParentIndices(index))
        {
            if (IsPaddedIndex(parent))
            {
                return true;
            }
        }
    ...
    TransformedDomain TransformedDomain::Fuse(const std::vector<TransformedDomain>& domains, const std::vector<std::vector<Index>>& indexCorrespondences)
    {
        ...
        {
            throw std::logic_error("Index correspondences don't match number of domains");
        }

        // First concatenate all the info from both domains
        ...
        {
            // TODO: use the index with the largest range instead of defaulting to domain1's index.
            // currently, this logic assumes range(i1) == max(range(i1), range(i2))
            ...
            for (unsigned idx = 1; idx < correspondingIndices.size(); ++idx)
            {
                ...
                // TODO: remove this assert once the index with the largest range is used as the fused index
                assert(result._indices[i2].range <= result._indices[i1].range && "Fused index range is too small");

                result._dimensions.erase(
                    std::remove_if(
                        result._dimensions.begin(), result._dimensions.end(), [i2](const Index& i) { return i == i2; }),
    ...
    void TransformedDomain::AddDimension(const Index& dimension, const Range& range)
    {
        ...
        _indices[dimension] = { {}, range, /*parents=*/{} };
    }
    ...
    AffineConstraints TransformedDomain::GetConstraints() const
    {
        ...
        // For each index:
        //   add its range (lower bound, upper bound)
        ...
        for (const auto& [index, indexInfo] : _indices)
        {
            ...
            if (!indexInfo.expr.IsIdentity())
            {
                constraints.AddConstraint(index, indexInfo.expr, [](AffineExpression expr) -> std::vector<int64_t>
                {
                    ...
                    else if (auto size = IsPadExpression(expr))
                    {
                        // front-padding: d1 = d0 - size
                        return {1, *size * -1};
                    }
