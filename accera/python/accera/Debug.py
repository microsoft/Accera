...
def add_check_allclose(package: Package, array: Array, atol: float=1e-5) -> Function:
    ...
    return package.add_function(nest, args=(actual, desired),
        base_name=f"_debug_check_allclose_{shape_str}")


def add_debugging_functions(package: Package, functions_to_args: Dict, atol: float=1e-5) -> Dict:
    ...
    for name, args in functions_to_args.items():
        result[name] = []
        for arg in args:
            sig = get_signature(arg)
            if sig not in available_fns:
                available_fns[sig] = add_check_allclose(package, arg, atol)
