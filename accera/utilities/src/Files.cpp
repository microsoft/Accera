...
namespace utilities
{
    ...
    void EnsureDirectoryExists(const std::string& path)
    {
        if (!DirectoryExists(path))
        {
            ...
            if (rc != 0)
            {
                throw accera::utilities::Exception(accera::utilities::FormatString("mkdir failed with error code %d", errno));
            }
        }
    }
    ...
    std::string GetWorkingDirectory()
    {
        ...
        if (rc != 0)
        {
            throw accera::utilities::Exception(accera::utilities::FormatString("error getting current working directory: %d", rc));
        }
        return utf8wd;
    }
...
#pragma warning(disable : 4996)
        ...
        throw accera::utilities::Exception("Could not find '" + name + "' in your PATH environment");
    }
} // namespace utilities
} // namespace accera
