...
# Section 5: Targets
Accera is a cross compiler since it can generate executable code for different target platforms that can be described using the `Target` class. Accera already supports many different targets, for example: 
```python
...
One of the benefits of the targets is that they provide a standard way of accessing useful constants. For example, we may want to split an iteration space dimension by the number of elements that fit in a vector register.
```python
