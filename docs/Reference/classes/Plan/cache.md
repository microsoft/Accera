...
`vectorize` | Whether to vectorize the cache operations. Defaults to `AUTO` will behave like `vectorize=True` if the loop nest has any vectorized loop via `plan.vectorize(index)` or `vectorize=False` if the loop nest has no vectorized loops. | `bool`
