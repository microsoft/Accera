...
`sample` | A number to limit the size of the parameter grid. The grid will be randomly sampled. | integer
...
Define a lambda or function to filter out combinations from the parameter grid. The arguments to the filter are the values of a parameter combination, and it should return True if the combination should be included, and False otherwise:
...
Parameter grids can result in a large number of possible combinations. We can limit the number of parameters by randomly sampling:
